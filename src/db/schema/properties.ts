...
export const properties = pgTable(
  'properties',
  {
    ...
    mailCity: varchar('mail_city', { length: 128 }),
    mailState: varchar('mail_state', { length: 2 }),
    mailZip: varchar('mail_zip', { length: 10 }),
    mailVacant: boolean('mail_vacant'),

    // PropertyRadar CSV — purchase, tax, loan details
    purchaseDate: date('purchase_date'),
    purchaseAmountCents: bigint('purchase_amount_cents', { mode: 'number' }),
    purchaseSeller: text('purchase_seller'),
    taxPerYearCents: bigint('tax_per_year_cents', { mode: 'number' }),
    estTaxPercent: numeric('est_tax_percent', { precision: 5, scale: 2 }),
    taxDelinquentCents: bigint('tax_delinquent_cents', { mode: 'number' }),
    estEquityPercent: numeric('est_equity_percent', { precision: 5, scale: 2 }),
    estOpenLoansCents: bigint('est_open_loans_cents', { mode: 'number' }),
    cltvPercent: numeric('cltv_percent', { precision: 5, scale: 2 }),
    estOpenLoansCount: integer('est_open_loans_count'),
    firstLoanPurpose: text('first_loan_purpose'),
    firstLoanType: text('first_loan_type'),
    firstLoanDate: date('first_loan_date'),
    firstLoanAmountCents: bigint('first_loan_amount_cents', { mode: 'number' }),
    hudRent: integer('hud_rent'),
