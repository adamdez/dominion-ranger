...
async function seedAdmin() {
  try {
    const [existing] = await db.select().from(users).where(eq(users.email, 'adam@dominionhomes.com')).limit(1);
    if (existing) {
      ...
      const admin = await createUser({ email: 'adam@dominionhomes.com', password: 'changeme123', name: 'Adam DesJardin', role: 'ADMIN', phone: '5093090434' });
